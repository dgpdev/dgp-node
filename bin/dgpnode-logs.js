...
const dgpnode_logs = require('commander');
const fs = require('fs');
...
let address = null;
if (dgpnode_logs.remote) {
  address = dgpnode_logs.remote.split(':')[0];
  if (dgpnode_logs.remote.split(':').length > 1) {
    port = parseInt(dgpnode_logs.remote.split(':')[1], 10);
  }
}
...
utils.connectToDaemon(port, function(rpc, sock) {
  ...
  rpc.status((err, shares) => {
    ...
    for (let i = 0; i < shares.length; i++) {
      if (shares[i].id === dgpnode_logs.nodeid) {
    ...
    const fslogger = new FsLogger(logFileDir, dgpnode_logs.nodeid);
    ...
    setInterval(function() {
      if (currentFile !== fslogger._todaysFile()) {
        ...
        if (!utils.existsSync(fslogger._todaysFile())) {
          console.error(`\n  no logs to show for ${dgpnode_logs.nodeid}`);
          return sock.end();
        }

        logTail = new Tail(fslogger._todaysFile());
        let numLines = dgpnode_logs.lines
                     ? parseInt(dgpnode_logs.lines)
                     : 20;
