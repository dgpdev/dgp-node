...
const dgpnode_daemon = require('commander');
...
let address = null;
if (dgpnode_daemon.remote) {
  address = dgpnode_daemon.remote.split(':')[0];
  if (dgpnode_daemon.remote.split(':').length > 1) {
    port = parseInt(dgpnode_daemon.remote.split(':')[1], 10);
  }
}

utils.checkDaemonRpcStatus(port, (isRunning) => {
  if (dgpnode_daemon.status) {
    console.info(`\n  * daemon ${isRunning ? 'is' : 'is not'} running`);
    process.exitCode = isRunning ? 0 : 3;
  } else if (isRunning) {
      return console.info('\n  * daemon is already running');
  } else {
    if (dgpnode_daemon.foreground) {
